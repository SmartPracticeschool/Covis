...
def table_plot(df):
    ...
    for locations in locations_list:
        for ids in np.where(df.Location == locations):
            ...
            sentiment_1_dict[locations.title()] = round(
                (total_pos_sentiment/total_setiments)*100, 2)
            sentiment_neg_1_dict[locations.title()] = round(
                (total_neg_sentiment/total_setiments)*100, 2)
            sentiment_0_dict[locations.title()] = round(
                (total_neutral_sentiment/total_setiments)*100, 2)
            
    return dict(Location_list = list(sentiment_1_dict.keys()),
                Positive_sentiments = list(sentiment_1_dict.values()),
                Negative_sentiments = list(sentiment_neg_1_dict.values()),
                Neutral_sentiments = list(sentiment_0_dict.values())
                )
